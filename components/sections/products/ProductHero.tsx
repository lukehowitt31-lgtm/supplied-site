...
export function ProductHero({ tag, headline, subheadline, intro }: ProductHeroProps) {
  return (
    <section className="bg-supplied-ink text-white pt-[140px] pb-14 sm:pb-16 relative overflow-hidden">
      ...
      />
      <Container className="relative z-10 text-center max-w-[760px]">
        <div className="mb-4 animate-slide-up opacity-0 [animation-delay:0.1s]">
          <Tag color="amber">{tag ?? "Our Products"}</Tag>
        </div>
        <AccentHeading
          as="h1"
          text={headline ?? "Every material, every format.|Sourced for you."}
          className="text-[clamp(34px,4.2vw,52px)] font-extrabold leading-[1.08] tracking-[-0.025em] mb-4 animate-slide-up opacity-0 [animation-delay:0.2s]"
          accentClassName="text-supplied-amber-bright"
        />
        <p className="text-[15px] sm:text-[16px] text-white/55 leading-[1.6] max-w-[560px] mx-auto animate-slide-up opacity-0 [animation-delay:0.3s]">
        ...
        {intro && (
          <p className="text-[13px] sm:text-[14px] text-white/40 leading-[1.6] max-w-[600px] mx-auto mt-4 animate-slide-up opacity-0 [animation-delay:0.4s]">
            {intro}
          </p>
        )}
      </Container>
